...
async def run_my_simple_evaluation() -> None:
    ...
    await eval_dataset(
        data=lambda: [
            ...
            TestInput[QueryInputs](name="Test Case 5: Question About Features", inputs={"query": "What features are available?"}),
            TestInput[QueryInputs](name="Test Case 6: Technical Support", inputs={"query": "I need help with API integration"}),
            TestInput[QueryInputs](name="Test Case 7: Long Query", inputs={"query": "This is a much longer query that contains multiple sentences and should test how the system handles more complex input with various punctuation marks, numbers like 123, and special characters!"}),
            TestInput[QueryInputs](name="Test Case 8: Multilingual", inputs={"query": "Bonjour, comment ça va?"}),
            TestInput[QueryInputs](name="Test Case 9: Code Query", inputs={"query": "How do I use the eval_dataset function in Python?"}),
            TestInput[QueryInputs](name="Test Case 10: Numeric Input", inputs={"query": "12345"}),
            TestInput[QueryInputs](name="Test Case 11: Special Characters", inputs={"query": "!@#$%^&*()_+-=[]{}|;:,.<>?"}),
            TestInput[QueryInputs](name="Test Case 12: Whitespace Test", inputs={"query": "   spaces   and   tabs   "}),
        ],
