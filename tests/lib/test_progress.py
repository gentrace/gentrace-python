...
class TestRichProgressReporter:
    ...
    def test_start(self, mock_progress_class: Any, _mock_console_class: Any) -> None:
        ...
        assert 'Starting...' in call_args[1]['description']
    
    @patch('gentrace.lib.progress.Console')
    @patch('gentrace.lib.progress.Progress')
    def test_update_current_test(self, mock_progress_class: Any, _mock_console_class: Any) -> None:
        ...
        # Verify update was called with the test name
        update_calls = mock_progress.update.call_args_list
        assert len(update_calls) > 0
        last_call = update_calls[-1]
        assert 'Running: Login Test' in last_call[1]['description']
    
    @patch('gentrace.lib.progress.Console')
    @patch('gentrace.lib.progress.Progress')
    def test_increment(self, mock_progress_class: Any, _mock_console_class: Any) -> None:
        ...
        assert 'Completed: Test 1' in last_call[1]['description']
    
    @patch('gentrace.lib.progress.Console')
    @patch('gentrace.lib.progress.Progress')
    def test_stop(self, mock_progress_class: Any, _mock_console_class: Any) -> None:
        ...
        # Verify final update message
        update_calls = mock_progress.update.call_args_list
        last_call = update_calls[-1]
        assert 'Evaluation complete' in last_call[1]['description']
    
    def test_format_test_name_short(self) -> None:
        """Test formatting short test names."""
        reporter = RichProgressReporter()
        
        # Short name should be padded
        formatted = reporter._format_test_name("Test", max_length=10)
        assert formatted == "Test      "
        assert len(formatted) == 10
    
    def test_format_test_name_exact(self) -> None:
        """Test formatting test names of exact length."""
        reporter = RichProgressReporter()
        
        # Exact length should remain unchanged
        formatted = reporter._format_test_name("Test123456", max_length=10)
        assert formatted == "Test123456"
        assert len(formatted) == 10
    
    def test_format_test_name_long(self) -> None:
        """Test formatting long test names with truncation."""
        reporter = RichProgressReporter()
        
        # Long name should be truncated with ellipsis
        formatted = reporter._format_test_name("This is a very long test name", max_length=10)
